...
Validates an entry code for a **specific group** and enrolls the authenticated student. The group id is required in the URL so codes cannot be probed across all groups.
...
| --------- | ---- | ----------- |
| `groupId` | integer | Public group ID (includes `GROUP_RESPONSE_GROUP_ID_OFFSET`). Values below the offset are accepted as raw DB ids for backwards compatibility. |

**Query parameters:**

| Parameter | Type | Rules | Description |
| --------- | ---- | ----- | ----------- |
| `code` | string | Exactly 6 characters | Entry code for **this group only** (from `POST /api/groups/generate-code`). |
...
**Behaviour:** Loads the group by id **and** `entry_code = code`. Wrong group, missing group, or wrong code return `enrollmentId: -2` (same code for all to prevent enumeration).

**Response:** `200 OK` with JSON body (same shape as **`POST /api/groups/:groupId/enroll`**):

| Field | Type | Description |
| ----- | ---- | ----------- |
| `statusCode` | integer | Example contract uses `200`. |
| `enrollmentId` | integer | New or existing enrollment id on success. Negative on error: `-1` not authorized, `-2` invalid group/code pair, `-3` DB error. |
| `groupId` | integer (optional) | Public group ID on success only. |

**Example**

```http
GET /api/groups/100137/invite?code=ABCDEF&auth=<token> HTTP/1.1
Host: 127.0.0.1:8080
X-Browser-ID: <BrowserUUID>
```
