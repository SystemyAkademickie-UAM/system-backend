...
export class MagicLinkUserService {
  ...
  async resolveEmailMagicLinkTarget(
    ...
    const emailOrganizationIds = await this.listActiveEmailOrganizationIdsForUser(userId);
  ...
  async resolveEligibleUserIdForMagicLink(
    ...
    const target = await this.resolveEmailMagicLinkTarget(emailRaw, bootstrapEmail);
  ...
  private async listActiveEmailOrganizationIdsForUser(userId: number): Promise<number[]> {
    const rows = await this.accountRepository
      ...
      .andWhere('org.login_method = :loginMethod', { loginMethod: ORGANIZATION_LOGIN_METHOD_EMAIL })
